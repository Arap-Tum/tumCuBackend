...
const FLeader = require("../models/formerLeaders.js");
...
const getLeaders = async (req, res) => {
  try {
    const leaders = await FLeader.find().sort({ createdAt: -1 });
...
const getLeaderById = async (req, res) => {
  try {
    const leader = await FLeader.findById(req.params.id);
    if (!leader) {
...
const updateLeader = async (req, res) => {
  try {
    ...
    const updatedItem = await FLeader.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
const deleteLeader = async (req, res) => {
  try {
    ...
    const deletedItem = await FLeader.findByIdAndDelete(id);
